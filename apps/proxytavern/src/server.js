import "dotenv/config";
import express from "express";
import cors from "cors";
import path from "node:path";
import crypto from "node:crypto";
import { fileURLToPath } from "node:url";
import { applyBlockRules } from "./jsonpath-lite.js";
import { createToken, getConfig, listSessions, resetConfig, saveSession, setConfig, verifyToken } from "./store.js";

export function createApp() {
  const app = express();
  app.use(cors());
  app.use(express.json({ limit: "5mb" }));

  const __filename = fileURLToPath(import.meta.url);
  const __dirname = path.dirname(__filename);
  const WEB_DIR = path.resolve(__dirname, "../web");

  function auth(req, res, next) {
    if (req.path.startsWith("/api/")) return next();
    const token = (req.header("authorization") || "").replace(/^Bearer\s+/i, "");
    if (!verifyToken(token)) return res.status(401).json({ error: "invalid_token" });
    return next();
  }

  app.get("/health", (_req, res) => res.json({ ok: true }));

  app.get("/api/config", (_req, res) => {
    res.set("Cache-Control", "no-store");
    res.json(getConfig());
  });

  app.post("/api/config", (req, res) => {
    try {
      const updated = setConfig(req.body || {});
      res.json(updated);
    } catch (err) {
      res.status(422).json({
        error: "validation_error",
        message: err instanceof Error ? err.message : "Invalid config payload"
      });
    }
  });

  app.post("/api/config/reset", (_req, res) => {
    res.json(resetConfig());
  });

  app.post("/api/config/test-connection", async (_req, res) => {
    const cfg = getConfig();
    try {
      const controller = new AbortController();
      const timeout = setTimeout(() => controller.abort(), 5000);
      const response = await fetch(cfg.upstreamUrl, {
        method: "OPTIONS",
        signal: controller.signal
      });
      clearTimeout(timeout);
      res.json({ ok: response.ok, status: response.status, upstreamUrl: cfg.upstreamUrl });
    } catch (err) {
      res.status(502).json({
        ok: false,
        upstreamUrl: cfg.upstreamUrl,
        message: err instanceof Error ? err.message : "connection failed"
      });
    }
  });

  app.post("/api/token/generate", (_req, res) => {
    const token = createToken();
    res.json({ token });
  });

  app.get("/api/sessions", (_req, res) => {
    res.json(listSessions());
  });

  app.post("/v1/chat/completions", auth, async (req, res) => {
    const incoming = req.body;
    const config = getConfig();
    const transformed = applyBlockRules(incoming, config.blockedJsonPaths || []);

    const outboundHeaders = { "Content-Type": "application/json" };
    if (config.upstreamAuth?.enabled && process.env.UPSTREAM_AUTH_TOKEN) {
      outboundHeaders[config.upstreamAuth.headerName || "Authorization"] = process.env.UPSTREAM_AUTH_TOKEN;
    }

    const startedAt = new Date().toISOString();
    const upstreamResp = await fetch(config.upstreamUrl, {
      method: "POST",
      headers: outboundHeaders,
      body: JSON.stringify(transformed)
    });

    const upstreamBody = await upstreamResp.json().catch(async () => ({ raw: await upstreamResp.text() }));

    saveSession({
      id: crypto.randomUUID(),
      startedAt,
      incoming,
      transformed,
      upstreamBody,
      status: upstreamResp.status
    });

    res.status(upstreamResp.status).json(upstreamBody);
  });

  app.use("/ui", express.static(WEB_DIR));
  app.get("/", (_req, res) => res.redirect("/ui"));

  return app;
}

if (process.env.NODE_ENV !== "test") {
  const app = createApp();
  const port = Number(process.env.PORT || "8787");
  const host = process.env.HOST || "0.0.0.0";
  app.listen(port, host, () => {
    console.log(`ProxyTavern listening on http://${host}:${port}`);
  });
}
